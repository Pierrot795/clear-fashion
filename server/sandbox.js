...
async function sandbox (eshop = 'https://www.dedicatedbrand.com/en/men') { //cet url là par defaut <=> page scrapée par defaut
  try {
    console.log(`🕵️‍♀️  browsing ${eshop} source`); //s'affiche dans le serveur node

    const products = await adressebrand.scrape(eshop); //stocke le resultat du scraping sur la page definie par le lien
...
//retrieveurls();
//sandbox(eshop);
sandbox('https://adresse.paris/630-toute-la-collection?id_category=630&n=109');
